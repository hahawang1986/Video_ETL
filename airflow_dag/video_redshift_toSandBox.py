...
with DAG(
    dag_id='load_redshift_data_to_Sandbox_dag',
...
############### move redshift analytical data to SandBox for downstream visualization ###################
# define Redshift UNLOAD SQL 
# this sql is to generate a table for  "The program with the highest click-through rate per hour"
    def unload_redshift_to_s3():
        # S3 and Redshift variables
        s3_bucket = 'video-analytical-bucket'
        s3_prefix = 'data-warehouse/'
        iam_role = 'arn:aws:iam::471112748839:role/service-role/AmazonRedshift-CommandsAccessRole-20240528T153201'
        ...
        current_date = datetime.now()
        current_date_str = current_date.strftime('%Y-%m-%d')
        s3_path = f's3://{s3_bucket}/highest_click-through_rate_per_hour/{current_date_str}/'
        
        # 定义 Redshift UNLOAD SQL 语句  实际上可以写在.sql文件里，调用。sql文件这样比较简洁
        unload_sql = f'''
        UNLOAD ('
        WITH topic_counts AS (
            SELECT
                dimdate.hour,
                factvideo.topic_id,
                COUNT(factvideo.topic_id) AS topic_number,
                ROW_NUMBER() OVER (PARTITION BY dimdate.hour ORDER BY COUNT(factvideo.topic_id) DESC) AS rn
            FROM factvideo
            INNER JOIN dimdate ON dimdate.datetime = factvideo.datetime
            GROUP BY dimdate.hour, factvideo.topic_id
        )
        SELECT 
            hour, 
            topic_counts.topic_id, 
            topic_number, 
            dimvideotopic.videotopic
        FROM topic_counts
        INNER JOIN dimvideotopic ON dimvideotopic.topic_id = topic_counts.topic_id
        WHERE rn = 1
        ORDER BY hour
        ')
        TO '{s3_path}'
    ...
    redshift_unload_task = PythonOperator(
    task_id='redshift_unload_task',
    python_callable=unload_redshift_to_s3,
    )

#############  sequence #######################
list_files >> move_files >> start_glue_job_raw >> start_glue_job_curated >> [json_to_redshift_task,csv_to_redshift_task] >> redshift_unload_task
